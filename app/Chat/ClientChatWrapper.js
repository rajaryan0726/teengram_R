...
export default function ClientChatWrapper({ initialConversations, currentUserId, initialActiveChatId }) {
    ...
    // 2. Find the currently active chat object
    // activeChatId is now a guaranteed Conversation ID (or null)
    const activeChat = conversations.find(chat => chat._id.toString() === activeChatId) 
    ...
    // 3. Logic to update the list if a new message arrives (deferred for later)
